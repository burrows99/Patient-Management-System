...
class OllamaTriageAgent(TriageSystem):
    ...
    def __init__(self, model_name: str = "phi:2.7b", base_url: str = "http://localhost:11434", disable_fallback: bool = True):
        ...
        """
        # Load configuration from root config folder (with sensible defaults)
        self.config = self._load_config()
        self.model_name = model_name or self.config.get('model_name', 'phi:2.7b')
    ...
    def _extract_text_from_result(self, result: Any, http_ms: int) -> str | None:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        cfg_path = project_root / 'config' / 'ollama_config.yaml'
        if cfg_path.exists():
            try:
